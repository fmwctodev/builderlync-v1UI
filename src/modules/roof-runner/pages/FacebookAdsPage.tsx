...
const FacebookAdsPage: React.FC = () => {
    ...
    const handleCallback = async (code: string) => {
        try {
            ...
            const response = await fetch(
                `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3200/api'}/social-ads/facebook/callback?code=${code}`,
                {
            ...
            if (response.ok) {
                ...
                setError(data.message || 'Failed to connect Facebook Ads account');
                setLoading(false);
            }
        } catch (err) {
            setError('Error connecting to Facebook Ads');
            setLoading(false);
        }
    };
    ...
    const fetchCampaigns = async () => {
        try {
            setLoading(true);
            const response = await fetch(
                `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3200/api'}/social-ads/facebook/campaigns`,
                {
                    headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
                }
            );
            ...
            if (response.ok) {
                ...
                const data = await response.json().catch(() => ({}));
                setError(data.message || 'Not connected to Facebook Ads');
            }
        } catch (err) {
        ...
        <div className="p-8 space-y-6">
            ...
            {campaigns.length === 0 && (
                <div className="bg-white dark:bg-gray-800 rounded-lg p-12 border border-gray-200 dark:border-gray-700 text-center">
                    <p className="text-gray-600 dark:text-gray-400">No campaigns found for your Facebook Ads account</p>
                </div>
            )}
        </div>
    );
