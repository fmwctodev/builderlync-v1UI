              ...
              {AI_PROPOSAL_TEMPLATES.map((tpl) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <FileText size={16} className={selectedTemplate?.id === tpl.id ? 'text-primary-600' : 'text-gray-400'} />
                    {selectedTemplate?.id === tpl.id && (
                      <CheckCircle size={14} className="text-primary-600" />
                    )}
                  </div>
                  <div className={`text-sm font-semibold mb-1 ${selectedTemplate?.id === tpl.id ? 'text-primary-800 dark:text-primary-200' : 'text-gray-900 dark:text-white'}`}>
                    {tpl.name}
                  </div>
