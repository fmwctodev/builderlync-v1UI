...
export default function AiProposalGenerator() {
  ...
  function selectTemplate(template: AiProposalTemplate) {
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  {i < stepLabels.length - 1 && (
                    <div className={`w-4 h-px ${isDone ? 'bg-blue-300 dark:bg-blue-700' : 'bg-gray-200 dark:bg-gray-600'}`} />
                  )}
                </React.Fragment>
              );
            })}
                ...
                <div className="relative" ref={contactDropdownRef}>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-9 pr-4 py-2.5 border rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm ${
                        selectedContact
                ...
                {selectedContact && (
                  <div className="mt-2 flex items-center gap-2 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-blue-800 dark:text-blue-200 truncate">{selectedContact.full_name}</div>
                      {selectedContact.email && (
                        <div className="text-xs text-blue-600 dark:text-blue-400 truncate">{selectedContact.email}</div>
                      )}
                    </div>
                    <button onClick={clearContact} className="text-blue-400 hover:text-blue-600 dark:hover:text-blue-300 text-xs shrink-0">
                      Change
                    </button>
                  </div>
                )}
              ...
              {AI_PROPOSAL_TEMPLATES.map((tpl) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <FileText size={16} className={selectedTemplate?.id === tpl.id ? 'text-blue-600' : 'text-gray-400'} />
                    {selectedTemplate?.id === tpl.id && (
                      <CheckCircle size={14} className="text-blue-600" />
                    )}
                  </div>
                  <div className={`text-sm font-semibold mb-1 ${selectedTemplate?.id === tpl.id ? 'text-blue-800 dark:text-blue-200' : 'text-gray-900 dark:text-white'}`}>
                    {tpl.name}
                  </div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">{tpl.description}</div>
                </button>
              ))}
            </div>
            ...
            {!generationError ? (
              <>
                <div className="relative">
                  ...
                  <Loader2 className="absolute -inset-1 w-22 h-22 text-blue-500 animate-spin opacity-40" style={{ width: 88, height: 88, top: -4, left: -4 }} />
                </div>
                <div>
                  ...
                  {STATUS_MESSAGES.map((_, i) => (
                    <div
                      ...
                      className={`h-1.5 rounded-full transition-all duration-500 ${
                        i <= statusIndex ? 'w-6 bg-blue-500' : 'w-2 bg-gray-200 dark:bg-gray-700'
                      }`}
                    />
                  ))}
                </div>
