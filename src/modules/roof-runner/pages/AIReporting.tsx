...
import { aiReportsApi } from '../services/aiReportsApi';
...
export function AIReporting() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const orgIdFromStorage = localStorage.getItem('currentOrganizationId');
        const userIdFromStorage = localStorage.getItem('currentUserId');
        
        if (orgIdFromStorage) setOrgId(orgIdFromStorage);
        if (userIdFromStorage) setUserId(userIdFromStorage);
      } catch (err) {
        console.error('Failed to get org/user info:', err);
      }
    })();
  }, []);
  ...
  const loadRecentReports = useCallback(async () => {
    ...
    try {
      const data = await aiReportsApi.getAIReports(orgId, { status: 'complete' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await aiReportsApi.generateReport(orgId, userId, {
        prompt: userText,
        scope,
      ...
      const final = await aiReportsApi.pollReportStatus(result.report_id!, (updated) => {
