...
import type {
  JobCamTemplate, JobCamTemplateItem, JobPhoto
} from '../../../types/jobCam';
...
const ChecklistTab: React.FC<Props> = ({ photos }) => {
          ...
          <div key={category} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${categoryColor[category] ?? 'bg-gray-100 text-gray-600'}`}>
                  {category}
                </span>
              ...
              <span className="text-xs font-bold text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 px-2 py-1 rounded-md">
                ...
                {categoryItems.map(item => {
                  ...
                  return (
                    <div key={item.id} className="flex items-center gap-4 px-5 py-4">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 animate-in zoom-in duration-300 ${
                        status === 'complete'
                          ? 'bg-green-50 dark:bg-green-900/20 shadow-sm border border-green-100 dark:border-green-800'
                          : status === 'missing'
                          ? 'bg-red-50 dark:bg-red-900/20 shadow-sm border border-red-100 dark:border-red-800'
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-bold ${status === 'complete' ? 'text-gray-400 dark:text-gray-500 line-through' : 'text-gray-900 dark:text-white'}`}>
                          {item.name}
                        </p>
                        {item.description && (
                      ...
                      {item.is_required && status !== 'complete' && (
                        <span className="text-[10px] bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-2 py-1 rounded font-bold uppercase tracking-wider">Required</span>
                      )}
                    </div>
                  );
                })}
