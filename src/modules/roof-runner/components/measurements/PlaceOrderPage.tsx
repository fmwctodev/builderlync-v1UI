...
const PlaceOrderPage: React.FC<PlaceOrderPageProps> = ({ onOrderComplete, onBack }) => {
  ...
  const loadConnectionStatus = async () => {
    try {
      setLoading(true);
      const status = await eagleViewService.getConnectionStatus();
  ...
  if (step === 'product-selection') {
    ...
    return (
      <div className="space-y-6">
        ...
        {/* We keep the address search functionality but style the results area to match screenshot */}
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          {/* Keeping the search active so user can actually search */}
          {/* <div className="mb-6">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2 uppercase tracking-wide">Property Location</h3>
            <div className="p-4 bg-gray-100 border border-gray-300 rounded text-gray-500 text-center">
              Address Search Map (Temporarily Disabled for Debugging)
              <button className="block mx-auto mt-2 text-blue-600 underline" onClick={() => handleAddressSelect('Test Address', null)}>Use Test Address</button>
            </div>
          </div> */}

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Property Data Preview Card */}
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-8 text-center flex flex-col items-center justify-center min-h-[200px]">
              <div className="flex items-center gap-2 mb-4 w-full">
                <h4 className="font-semibold text-gray-900 dark:text-white">Property Data Preview</h4>
                <span className="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded">Basic</span>
              </div>
              <p className="text-sm text-gray-500 w-full text-left mb-8">Fast roof area estimate for quick budgeting.</p>

              <div className="flex flex-col items-center text-gray-400">
                <div className="w-12 h-12 mb-3 rounded-full bg-gray-100 flex items-center justify-center">
                  <Wallet className="w-6 h-6 text-gray-300" /> {/* Placeholder icon */}
                </div>
                <p className="text-sm">Select an address to preview roof data</p>
              </div>
            </div>
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-8 text-center flex flex-col items-center justify-center min-h-[200px]">
              <div className="w-full text-left mb-8">
                <h4 className="font-semibold text-gray-900 dark:text-white">Property Imagery</h4>
              </div>
              <div className="flex flex-col items-center text-gray-400 mt-4">
                <div className="w-12 h-12 mb-3 rounded-full bg-gray-100 flex items-center justify-center">
                  <Download className="w-6 h-6 text-gray-300" /> {/* Placeholder icon */}
                </div>
                <p className="text-sm">Select an address to view property imagery</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-6 pb-24"> {/* Added padding-bottom for sticky footer */}
            ...
            {selectedProducts.map(id => {
              ...
              return (
                <div key={id} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-0 text-sm">
                  ...
                  <span className="font-medium">{prod?.credits} Credits</span>
                </div>
              )
            })}
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-4 h-4 text-gray-400"><FileText className="w-full h-full" /></div> {/* Icon placeholder */}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              {!hasSufficient && <span className="text-[10px] bg-red-100 text-red-700 px-1.5 py-0.5 rounded border border-red-200">1 blocking</span>}
          ...
          <div className="p-4 space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between p-2 bg-green-50 border border-green-200 rounded text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-green-700">Using Credits</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between p-2 bg-green-50 border border-green-200 rounded text-sm">
                  <div className="flex items-center gap-2">
                    <Check className="w-4 h-4 text-green-600" />
                    <div>
                      <p className="font-medium text-gray-900">Upgrade Configuration</p>
                      <p className="text-xs text-green-700">Standard order flow</p>
                    </div>
                  </div>
                  <span className="text-xs font-bold text-green-700 uppercase">PASS</span>
                </div>
            ...
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase mb-2">PRICING & CREDITS</p>
              <div className="space-y-2">
                {hasSufficient ? (
                  <div className="flex items-center justify-between p-2 bg-green-50 border border-green-200 rounded text-sm">
                    <div className="flex items-center gap-2">
                ...
                <div className="flex items-center justify-between p-2 bg-green-50 border border-green-200 rounded text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-green-700">No promo code applied</p>
                    </div>
                  </div>
                  <span className="text-xs font-bold text-green-700 uppercase">PASS</span>
                </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 z-40 flex justify-between items-center px-8">
          ...
          <button
            ...
            disabled={!hasSufficient || isSubmitting}
            className={`px-6 py-2 rounded-lg font-medium text-white shadow-sm flex items-center gap-2 ${hasSufficient ? 'bg-gray-200 text-gray-800 hover:bg-gray-300' : 'bg-gray-200 text-gray-400 cursor-not-allowed'
              }`}
          >
