...
export async function fetchJobsWithMedia(): Promise<JobWithMediaSummary[]> {
  ...
  const photosByJob = (photos ?? []).reduce<Record<number, { job_id: number | null; review_status: ReviewStatus; capture_date: string; thumbnail_url: string | null; file_url: string }[]>>((acc, p) => {
