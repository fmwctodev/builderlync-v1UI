...
import { Search, TrendingUp, MousePointerClick, Eye, DollarSign, BarChart3, Loader2, RefreshCw, Settings } from 'lucide-react';
import { useCurrentOrganization } from '../../../shared/context/OrgContext';
import {
  getGoogleAdsAccount,
  getGoogleAdsKPIs,
  getGoogleAdsCampaignSummaries,
  getGoogleAdsDailySpend,
} from '../../../services/googleAds';
import type { GoogleAdsKPIs, GoogleAdsCampaignSummary, GoogleAdsAccount } from '../../../services/googleAds';
...
export function GoogleAdsTab() {
  ...
  const [account, setAccount] = useState<GoogleAdsAccount | null | undefined>(undefined);
  const [kpis, setKpis] = useState<GoogleAdsKPIs | null>(null);
  const [campaigns, setCampaigns] = useState<GoogleAdsCampaignSummary[]>([]);
  const [dailyData, setDailyData] = useState<{ date: string; spend: number; clicks: number }[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [acc, kpisData, campaignData, daily] = await Promise.all([
        getGoogleAdsAccount(currentOrganization.id),
        getGoogleAdsKPIs(currentOrganization.id, startDate, endDate),
        getGoogleAdsCampaignSummaries(currentOrganization.id, startDate, endDate),
        getGoogleAdsDailySpend(currentOrganization.id, startDate, endDate),
      ]);
      setAccount(acc);
      setKpis(kpisData);
    ...
    return (
      <div className="p-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 flex flex-col items-center text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">No Google Ads Account Connected</h3>
          <p className="text-gray-500 dark:text-gray-400 max-w-sm mb-6">
            Connect your Google Ads account to view real-time campaign performance, spend, and conversion data.
          </p>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                  ...
                  <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">CPC</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                {filtered.map((c) => (
                  <tr key={c.campaignId} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-right text-gray-700 dark:text-gray-300">{formatNum(c.impressions)}</td>
                    ...
                    <td className="px-4 py-3 text-sm text-right text-gray-700 dark:text-gray-300">{formatCurrency(c.cpc)}</td>
