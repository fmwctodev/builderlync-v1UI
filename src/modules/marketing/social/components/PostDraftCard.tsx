...
const PostDraftCard: React.FC<PostDraftCardProps> = ({
  ...
  if (publishStatus) {
    ...
    return (
      <div className="bg-gray-100/50 dark:bg-slate-700/50 rounded-xl border border-gray-300 dark:border-slate-600 p-4 opacity-80">
        <div className="flex items-center gap-2 mb-2">
          <ProviderIcon provider={draft.platform} size={18} />
          <span className="text-sm font-semibold text-gray-800 dark:text-slate-200">{config.label}</span>
          <span className="flex items-center gap-1 px-2 py-0.5 bg-green-500/20 text-green-400 text-xs rounded-full ml-auto">
            <CheckCircle size={11} />
            {publishedLabel}
          </span>
        </div>
        <p className="text-gray-500 dark:text-slate-400 text-sm line-clamp-2">{draft.hook}</p>
      </div>
    );
  }

  return (
    <div className="bg-gray-100 dark:bg-slate-700 rounded-xl border border-gray-300 dark:border-slate-600 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <ProviderIcon provider={draft.platform} size={18} />
        <span className="text-sm font-semibold text-gray-800 dark:text-slate-200">{config.label}</span>
        {draft.engagement_prediction > 0 && (
          <span className="ml-auto px-2 py-0.5 bg-primary-500/20 text-primary-500 text-xs rounded-full">
      ...
      {draft.hook && (
        <p className="text-gray-900 dark:text-white font-semibold text-sm leading-snug">{draft.hook}</p>
      )}
      {draft.body && (
        <p className="text-gray-700 dark:text-slate-300 text-sm leading-relaxed">{draft.body}</p>
      )}
      {draft.cta && (
        <p className="text-primary-500 dark:text-primary-400 text-sm italic">{draft.cta}</p>
      )}

      {draft.hashtags?.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {draft.hashtags.slice(0, 8).map((tag) => (
            <span key={tag} className="px-1.5 py-0.5 bg-gray-200 dark:bg-slate-600 text-gray-500 dark:text-slate-400 text-xs rounded">
              {tag}
            </span>
          ))}
        </div>
      ...
      {draft.visual_style_suggestion && (
        <p className="text-gray-400 dark:text-slate-500 text-xs italic">{draft.visual_style_suggestion}</p>
      )}

      <div className="flex items-center gap-2 text-xs">
        <span className={charPct > 100 ? 'text-red-400 font-bold' : charPct > 80 ? 'text-amber-400' : 'text-gray-400 dark:text-slate-500'}>
      ...
      {preloadedMedia.length > 0 && (
        <div className="flex gap-2 flex-wrap">
          {preloadedMedia.map((asset) => (
            <div key={asset.id} className="w-16 h-16 rounded-lg overflow-hidden bg-gray-200 dark:bg-slate-600">
      ...
      <div className="border-t border-gray-300 dark:border-slate-600 pt-3 space-y-3">
        {platformAccounts.length > 0 && (
          <div>
            <p className="text-xs text-gray-500 dark:text-slate-400 mb-2">Post to account</p>
            <div className="space-y-1.5">
              {platformAccounts.map((account) => (
                <label key={account.id} className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span className="text-xs text-gray-700 dark:text-slate-300 group-hover:text-gray-900 dark:group-hover:text-white truncate">{account.display_name}</span>
                </label>
              ))}
            </div>
          </div>
        ...
        {mode === 'schedule' && (
          <input
            ...
            className="w-full px-2 py-1.5 bg-gray-200 dark:bg-slate-600 border border-gray-400 dark:border-slate-500 rounded-lg text-xs text-gray-900 dark:text-white"
          />
        )}
        ...
        <button
          ...
          className="w-full py-2 bg-primary-600 hover:bg-primary-700 disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
        >
