...
const CreateCampaignModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl shadow-2xl w-full max-w-lg">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-slate-700">
          <h3 className="text-base font-semibold text-gray-900 dark:text-white">New Campaign</h3>
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="text-sm text-gray-500 dark:text-slate-400 block mb-1">Campaign name</label>
            <input
              value={name}
          ...
          <div>
            <label className="text-sm text-gray-500 dark:text-slate-400 block mb-1">Goal / brief</label>
            <textarea
              value={goal}
              onChange={(e) => setGoal(e.target.value)}
              placeholder="Describe the campaign goal and content style..."
              rows={3}
              className="w-full resize-none bg-gray-100 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-xl px-3 py-2 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-slate-500 focus:outline-none focus:border-primary-500"
            />
          </div>
          <div>
            <label className="text-sm text-gray-500 dark:text-slate-400 block mb-1">Posting frequency</label>
            <select
              value={frequency}
              onChange={(e) => setFrequency(e.target.value)}
              className="w-full bg-gray-100 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-xl px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-primary-500"
            >
          ...
          <div>
            <label className="text-sm text-gray-500 dark:text-slate-400 block mb-2">Target accounts</label>
            <AccountSelector
              accounts={accounts}
              groups={groups}
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-gray-200 dark:border-slate-700">
          <button onClick={onClose} className="flex-1 py-2 text-sm text-gray-500 dark:text-slate-400 hover:text-gray-800 dark:hover:text-slate-200 transition-colors">
            Cancel
          </button>
          <button
...
const CampaignCard: React.FC<{
  ...
  return (
    <div className="bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-5 hover:border-gray-300 dark:hover:border-slate-600 transition-colors">
      <div className="flex items-start justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white">{campaign.name}</h3>
          {campaign.goal && (
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-1 line-clamp-2">{campaign.goal}</p>
          )}
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="p-1.5 text-gray-400 dark:text-slate-500 hover:text-gray-800 dark:hover:text-slate-200 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-lg transition-colors"
            >
              <MoreVertical size={14} />
            </button>
            {menuOpen && (
              <div className="absolute right-0 top-full mt-1 bg-gray-100 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-xl shadow-xl overflow-hidden min-w-[140px] z-20">
                <button
      ...
      <div className="flex gap-2">
        <button
          ...
          disabled={generating}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 border border-gray-300 dark:border-slate-600 text-xs text-gray-700 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white rounded-xl transition-colors disabled:opacity-50"
        >
          {generating ? (
            <span className="w-3 h-3 border border-gray-400 dark:border-slate-400 border-t-white rounded-full animate-spin" />
          ) : (
            <Play size={11} />
          )}
          Generate posts
        </button>
        <button className="flex items-center gap-1 px-3 py-1.5 text-xs text-gray-500 dark:text-slate-400 hover:text-gray-800 dark:hover:text-slate-200 transition-colors">
          View posts
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-800/50">
        <div>
          ...
          <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">Manage content campaigns with autopilot scheduling</p>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-40 text-gray-400 dark:text-slate-500">
            ...
            <button
              onClick={() => setShowCreate(true)}
              className="mt-3 text-xs text-primary-500 dark:text-primary-400 hover:text-primary-400 dark:hover:text-primary-300 transition-colors"
            >
