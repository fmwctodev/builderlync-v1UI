  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 flex items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-500 capitalize">{account.provider.replace('_', ' ')}</p>
      ...
      <div className="flex items-center gap-2">
        {(account.status === 'error' || account.status === 'expired') && (
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs bg-slate-700 hover:bg-slate-600 border border-slate-600 text-slate-300 hover:text-white rounded-xl transition-colors disabled:opacity-50"
          >
        ...
        <button
          ...
          className="p-1.5 text-slate-500 hover:text-yellow-400 hover:bg-slate-700 rounded-lg transition-colors"
          title="Disconnect"
        >
          <XCircle size={14} />
        </button>
    ...
    <div className="flex flex-col h-full bg-slate-900">
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700 bg-slate-800/50">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Connect and manage your social media accounts</p>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto p-5 space-y-8">
        <section>
          <h3 className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-3">Connect New Account</h3>
            ...
            {PROVIDERS.map((provider) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs text-slate-400 capitalize">{provider.replace('_', ' ')}</span>
                  {isConnected && (
        ...
        {accounts.length > 0 && (
          <section>
            <h3 className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-3">Connected Accounts</h3>
            ...
            {disconnected.length > 0 && (
              <>
                <h4 className="text-xs text-slate-500 mt-4 mb-2">Disconnected</h4>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-medium text-slate-400 uppercase tracking-wider">Account Groups</h3>
            <button
          ...
          {groups.length === 0 ? (
            <p className="text-sm text-slate-500">No groups yet. Groups let you publish to multiple accounts at once.</p>
          ) : (
            <div className="space-y-2">
              {groups.map((g) => (
                <div key={g.id} className="bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 flex items-center gap-3">
                  <Users size={16} className="text-slate-500" />
                  <div className="flex-1">
                    <p className="text-sm text-slate-200">{g.name}</p>
                    {g.description && <p className="text-xs text-slate-500">{g.description}</p>}
                    <p className="text-xs text-slate-500">{g.account_ids.length} accounts</p>
                  </div>
                  <button
      ...
      {showGroupModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md p-6 space-y-4">
            <h3 className="text-base font-semibold text-white">Create Account Group</h3>
            <input
              value={newGroupName}
            ...
            />
            <div>
              <p className="text-xs text-slate-400 mb-2">Select accounts</p>
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {accounts.map((acc) => (
                  <label key={acc.id} className="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-700 cursor-pointer">
                    <input
                      type="checkbox"
