...
const MetricCard: React.FC<{
  ...
  <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 flex items-start gap-3">
    ...
    <div className="flex flex-col h-full bg-slate-900">
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700 bg-slate-800/50">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Performance metrics across all platforms</p>
        </div>
        <button
          onClick={handleRefresh}
          disabled={refreshing}
          className="flex items-center gap-1.5 px-3 py-2 bg-slate-700 hover:bg-slate-600 border border-slate-600 text-sm text-slate-300 hover:text-white rounded-xl transition-colors disabled:opacity-50"
        >
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-3 mb-8">
              <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 text-center">
                ...
                <p className="text-2xl font-semibold text-white">{fmt(agg.likes)}</p>
              </div>
              <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 text-center">
                <p className="text-xs text-slate-500 mb-1">Comments</p>
                <p className="text-2xl font-semibold text-white">{fmt(agg.comments)}</p>
              </div>
              <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 text-center">
                <p className="text-xs text-slate-500 mb-1">Saves</p>
                <p className="text-2xl font-semibold text-white">{fmt(agg.saves)}</p>
              </div>
            </div>

            <div className="bg-slate-800 border border-slate-700 rounded-2xl overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-700">
                <h3 className="text-sm font-medium text-slate-200">Top Posts</h3>
              </div>
              {data.length === 0 ? (
                  ...
                  {data.slice(0, 10).map((post) => {
                    ...
                    return (
                      <div key={post.id} className="px-4 py-3 flex items-center gap-4">
                        <p className="flex-1 text-sm text-slate-300 truncate">{post.body ?? post.hook_text ?? 'Post'}</p>
                        <div className="flex items-center gap-4 text-xs text-slate-500 flex-shrink-0">
                          ...
                          <span className="text-cyan-500 font-medium">{postAgg.engagementRate.toFixed(1)}%</span>
                        </div>
                      </div>
                    );
                  })}
