...
function StatusBadge({ status }: { status?: StormEventStatus }) {
  ...
  if (status === 'PROCESSING') {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 rounded-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <CloudLightning className="w-6 h-6 text-primary-600" />
            Storm Events
          </h1>
        ...
        {[
          {
            label: 'Total Events',
            value: events.length,
            icon: <CloudLightning className="w-5 h-5 text-primary-600" />,
